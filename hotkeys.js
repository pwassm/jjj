...
window.HOTKEYS = [
  ...
  { label: 'W  /  R  /  T  (in fun mode)', group: 'Grid (window-capture)', scope: 'G', dev: false,
    ...
    desc: 'The three fun modes, one letter each, claimed ONLY while fun mode is on — f has raised the card, or something is already running. W = waterfall, R = ring, T = turnaround; each pressed again stops its own mode and puts the chooser back. OUTSIDE fun mode: t goes back to the Table (dev0836 briefly gave bare t to turnaround; dev0837 reverts that — t→Table from the grid is used constantly), and (dev0838) bare w does NOTHING here at all, swallowed rather than forwarded to the clipboard import, which belongs to the Table and the Ig screen where there is a row to import into. R is the exception — it has started the ring cold since dev0374 and still does. TURNAROUND itself: click any cell and it turns over on its LONG midline (a landscape cell about its horizontal one, a portrait cell about its vertical one) to show the back — the row’s tag chips in the top half, one per line with the longest chip sized to span the card, and the first five lines of its ftext below. Click again and it turns back, a video resuming from exactly the frame it stopped on. (dev0840) The picture turns at full brightness — nothing is ever painted over it — against a black-grey backdrop laid over the cell’s slot for the duration, and it is eased so that its APPARENT HEIGHT falls at a constant rate rather than its angle. That distinction is the whole of the long-running “it fades before it turns” bug: a cell shows cos(angle) of its height, so at a constant angular rate it is still 71% tall at the halfway point and then collapses to nothing in the last quarter of the time, which reads as a fade and not a turn. A box floats under cell 5c with the spin number (1-20, default 5): a turn lasts 2/n seconds — the default 5 gives 0.4s, 1 is the slowest at two full seconds — and the value is remembered for next time.' },
