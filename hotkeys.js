...
window.HOTKEYS = [
  ...
  { label: 'D  (in Modes) — Fold', group: 'Grid (window-capture)', scope: 'G', dev: false,
    ...
    desc: 'FOLD MODE — the fold, which used to be a saved LAYOUT of its own (16F), lent instead to any ordinary grid. (dev0846) The layout is retired: T’s Mark-Grid chooser has lost its “13 ⧉” chip and C can no longer resolve a `cells: ‘16F’` token, which now reads as the plain square the config actually is — the two that carried it, HermitCrabAndPix_d and BoxerCrabsVideos25_d, are full 5×5s and fold on D like anything else. c.json was not rewritten. On any plain square 4×4 or larger, D re-lays the cells already on screen into the fold’s ten-cell staircase and they fold exactly as the old 16F did: click a circle and four squares collapse into one, 10 → 7 → 4 → 1. (dev0845) A single CLICK, on pointerup — the circles used to answer only to dblclick, which is the one event this grid cannot rely on, since its cells preventDefault on pointerdown and that suppresses every synthesised mouse event. Under fold mode they were dead outright. D again (or R, or the ✕) puts the grid back. Two things differ from the old saved layout. The FOOTPRINT is borrowed — the fold only ever occupies 1a-4d, so a 5×5 simply uses its top-left 4×4 and leaves row 5 and column e out. And the BACK FACES have no 1aB / 3cB / 4dB keys to read, because an ordinary config has never heard of them; they come instead from three of the cells the staircase does not use, in reading order — 1c is block A’s back (the corner fold onto 2b), 1d is block B’s (the centre fold), 2d is block C’s (the corner fold onto 3c). Which is why the mode wants THIRTEEN cells: ten for the paper and three for what is written on the other side. 3a, 4a and 4b sit it out. It REFUSES on anything else and says why: the 17 / 19 and portrait layouts merge or reshape their middle, so 2b-4d are not there to fold, and 3×3 has too few cells. Nothing is written — _gridCurrentLayout keeps reporting the grid’s real layout, so a Ctrl+Alt+G save during a fold saves the grid, not the fold.' },
  ...
  { label: 'Q  /  Shift+Q', group: 'Grid (window-capture)', scope: 'G', dev: false,
    ...
    desc: '(dev0844) OUTSIDE the Modes menu only — while Modes has the keyboard q is Quiz. New embed — reload the hovered IG/TikTok cell (Shift+Q: every embed on the grid) so it can play in the cell again. Those embeds allow ONE inline play each; after it the caret only offers to open the post on their site. Mostly a manual override now: since dev0671 a played cell re-primes itself once its clip has run. Also on the cell’s right-click menu and the ↻ chip on an armed cell.' },
