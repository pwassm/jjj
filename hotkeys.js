...
window.HOTKEYS = [
  ...
  { key: 't', label: 'T', group: 'Screens', scope: 'global',
    desc: 'Return to the Table (saves an open E screen first). On the grid this is the constantly-used way back; only while the MODES menu has the keyboard does t mean Turn instead (dev0837/0844).',
    fn(ctx) {
  ...
  { key: 'w', label: 'W', group: 'Import & filter', scope: 'global',
    desc: 'Smart clipboard import — bare media links, or @channel + CSV. The Table and the Ig screen only, since those are where there is a row to import into. (dev0838/0844) On the GRID the letter never even reaches here: core.js swallows a bare w there, and gives it to WANDER while the Modes menu has the keyboard.',
    fn(ctx) {
  ...
  { label: 'M', group: 'Grid (window-capture)', scope: 'G', dev: false,
    impl: 'core.js window-capture (dev0844) → collection.js _gmModesKey',
    desc: 'MODES — raises the window listing everything the grid can be doing, one letter each: R = Regular (the plain grid), T = Turn, F = Fall, W = Wander, D = Fold. (dev0937) Q is no longer among them: it was a named stub standing in front of the real flash-card quiz (quizcells.js, dev0901), which now answers bare q on the grid in every state. It also says what a CLICK on a cell means in whatever is running, which was documented nowhere before dev0705. M is a MENU key and nothing else — it toggles the list and never stops anything; R is the off switch. That split is the dev0844 correction to dev0837, where f was the door in AND the way out, so "close this window" and "stop what I started" could not be done separately. (dev0937) The card gets out of the way the moment ANY choice is made — you asked for the waterfall to watch it, not to read about it, and a chooser that answers a choice by staying up has to be dismissed twice; M brings it back to switch. It used to reappear whenever a letter left nothing running, which kept the choice letters claimed for a second press at the cost of the menu turning up under your hand every time you turned something off. Esc hides it too, and leaves what is running alone. A floating ✕ under cell 5c does R’s job for anyone without a keyboard. Was F from dev0705 to dev0843 — bare f on the grid is now inert.' },

  { label: 'R  /  T  /  F  /  W  /  D  (in Modes)', group: 'Grid (window-capture)', scope: 'G', dev: false,
    impl: 'core.js window-capture (dev0844) → collection.js _gmChoiceKey',
    desc: 'The modes, one letter each, claimed ONLY while the Modes menu has the keyboard — M has raised the card, or something is already running. Outside it, t still goes back to the Table, d still opens the Dictionary and f / w are inert; those are the keys used constantly here, which is exactly why the choice letters borrow them rather than take them. R is the exception: it works cold, because "put this grid back to normal" must not require finding a menu first. — R REGULAR: stops every engine, unfolds a fold, drops the card; a no-op when the grid is already plain. T TURN: click any cell and it turns over on its LONG midline (a landscape cell about its horizontal one, a portrait cell about its vertical one) to show the back — the row’s tag chips in the top half, one per line, all at one medium size, shrunk only as far as a long chip needs to fit, and the first five lines of its ftext below. Click again and it turns back, a video resuming from exactly the frame it stopped on. (dev0841) The picture turns at full brightness — nothing is ever painted over it — against a black-grey backdrop laid over the cell’s slot for the duration. BOTH HALVES run the same easing on their own progress, committing to the turn at once and easing into the end; making them time-reverses of each other was the whole of the long-running "it fades before it turns" bug, since reversing a decisive-then-settling motion gives a dormant-then-vanishing one. The two halves hand over on the Web Animations API, so there is no hole at the midpoint. (dev0842) And the return turns the SAME WAY ROUND as the outward trip rather than unwinding: on a landscape cell the top goes away from you in both directions, on a portrait cell the right edge does — one card being turned over and back over, not a film run backwards. A box floats under cell 5c with the spin number (1-20, default 8): a turn lasts 2/n seconds — the default 8 gives 0.25s, 1 is the slowest at two full seconds — and the value is remembered for next time. F FALL: the cells come off the cliff, bounce and re-enter (this was w from dev0837 to dev0843). W WANDER: the cells travel round the grid (this was r); its variants are listed UNDER it on the card because each is a different answer to "what happens when I click a travelling cell" — plain wander plays/pauses it, 1 = cascade flies it to a free slot, 2 = swap glides it into another’s place; pressing the running variant’s own number returns to plain wander. D FOLD: see its own row.' },

  { label: 'D  (in Modes) — Fold', group: 'Grid (window-capture)', scope: 'G', dev: false,
    impl: 'core.js window-capture (dev0844) → fold16.js _fold16ModeToggle',
    desc: 'FOLD MODE — the fold, which used to be a saved LAYOUT of its own (16F), lent instead to any ordinary grid. (dev0846) The layout is retired: T’s Mark-Grid chooser has lost its “13 ⧉” chip and C can no longer resolve a `cells: ‘16F’` token, which now reads as the plain square the config actually is — the two that carried it, HermitCrabAndPix_d and BoxerCrabsVideos25_d, are full 5×5s and fold on D like anything else. c.json was not rewritten. (dev0937) On ANY grid of sixteen cells or more — it was plain squares only, on the reasoning that 17 / 19 merge their middle and the portrait grids are rectangles, so 2b-4d were not there to be folded. That confused the grid’s cells with the fold’s: the fold has always drawn its own 4×4 footprint and its own thirteen slots, and gridShow’s `src` hop already lets a slot take its media from a differently-named cell. So the only real question is whether the grid has enough pictures, and the answer is sixteen. D re-lays the cells already on screen into the fold’s ten-cell staircase and they fold exactly as the old 16F did, 10 → 7 → 4 → 1 — and (dev0937) IT DOES IT ITSELF, on a loop, from the moment the mode starts: fold the lower-right block, then the upper-left, then the centre, then open them again in reverse, half a second between folds. That order is the geometry, not a preference — the centre block contains 2b and 3c, so it can only close once the two corners are inside it. (dev0938) THE CIRCLES AND THE DOUBLE-CLICK ARE GONE with it — the three dots at the interior corners, their near-miss catcher, and the _fold16ClaimDoubleTap hook grid.js tested ahead of its own editor routes. Nothing operates the fold now, so a double-click on a folding grid means what it means on any other grid. The dev-only ⏱ pill still tunes the speed; the pause stays half a second whatever the folds are doing. D again (or R, or the ✕) puts the grid back the way it was — and (dev0938) really does: a fold in flight is now called off by a generation counter (_f16Gen), where before its rAF loop ran on to a finish that re-applied the fold’s centred SQUARE template over the plain grid R had just rebuilt. Manually that was a fraction of a second you had to hit on purpose; with the fold always in flight it was almost every time, and it read as “the cells stay square after leaving Fold”. Two things differ from the old saved layout. The FOOTPRINT is borrowed — the fold only ever occupies 1a-4d, so a 5×5 simply uses its top-left 4×4 and leaves row 5 and column e out. And the BACK FACES have no 1aB / 3cB / 4dB keys to read, because an ordinary config has never heard of them; they come instead from three of the cells the staircase does not use, in reading order — 1c is block A’s back (the corner fold onto 2b), 1d is block B’s (the centre fold), 2d is block C’s (the corner fold onto 3c). Which is why the mode wants THIRTEEN cells: ten for the paper and three for what is written on the other side. 3a, 4a and 4b sit it out. It refuses only on a grid too small to fill it, and says so with the count. Where the thirteen slots get their pictures is _fold16ModeBuildSrc: a square grid keeps the arrangement it had (1a-4d are really there) and only the backs borrow, with an empty spare handing on to the next filled cell anywhere on the grid; any other layout borrows all thirteen, walking its own cell list in its own reading order, filled cells first. Nothing is written — _gridCurrentLayout keeps reporting the grid’s real layout, so a Ctrl+Alt+G save during a fold saves the grid, not the fold.' },
  ...
  { label: 'Q  /  Shift+Q', group: 'Grid (window-capture)', scope: 'G', dev: false,
    ...
    desc: '(dev0937) On the grid in every state — the Modes menu no longer shadows it; and on a grid holding flash cards bare q is the QUIZ (quizcells.js), which answers first. Otherwise: new embed — reload the hovered IG/TikTok cell (Shift+Q: every embed on the grid) so it can play in the cell again. Those embeds allow ONE inline play each; after it the caret only offers to open the post on their site. Mostly a manual override now: since dev0671 a played cell re-primes itself once its clip has run. Also on the cell’s right-click menu and the ↻ chip on an armed cell.' },
  ...
  { label: 'Shift+drag on a cell', group: 'Gestures', scope: 'G', dev: true, helpSection: 'Gestures',
    ...
    desc: 'Pan the zoomed cell content to look around. Always transient (dev0948) — to KEEP a framing, Alt-click the subject instead' },

  { label: 'Alt+click a cell', group: 'Gestures', scope: 'G', dev: true, helpSection: 'Gestures',
    impl: 'grid.js gridSetCOI (dev0948)',
    desc: 'Centre of interest: the point you click is stored on the ROW and centred in the cell at any zoom, together with the cell zoom it was set at — which the cell then loads at, wherever it is placed (dev0948)' },
