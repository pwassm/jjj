...
window.HOTKEYS = [
  ...
  { label: 'W  /  R  /  T  (in fun mode)', group: 'Grid (window-capture)', scope: 'G', dev: false,
    ...
    desc: 'The three fun modes, one letter each, claimed ONLY while fun mode is on — f has raised the card, or something is already running. W = waterfall, R = ring, T = turnaround; each pressed again stops its own mode and puts the chooser back. OUTSIDE fun mode: t goes back to the Table (dev0836 briefly gave bare t to turnaround; dev0837 reverts that — t→Table from the grid is used constantly), and (dev0838) bare w does NOTHING here at all, swallowed rather than forwarded to the clipboard import, which belongs to the Table and the Ig screen where there is a row to import into. R is the exception — it has started the ring cold since dev0374 and still does. TURNAROUND itself: click any cell and it turns over on its LONG midline (a landscape cell about its horizontal one, a portrait cell about its vertical one) to show the back — the row’s tag chips in the top half, one per line with the longest chip sized to span the card, and the first five lines of its ftext below. Click again and it turns back, a video resuming from exactly the frame it stopped on. As the picture goes edge-on it shades toward the back face’s own grey against a black-grey backdrop, so the text arrives out of that dark instead of simply appearing. A box floats under cell 5c with the spin number (1-20, default 5): a turn lasts 2/n seconds — the default 5 gives 0.4s, 1 is the slowest at two full seconds — and the value is remembered for next time.' },
