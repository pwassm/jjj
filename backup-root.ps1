...
param(
    ...
    # one is not simply re-fetchable.
    # YTT\ is here for a sharper reason: it is gitignored too, so the two python
    # steps that PRODUCE those summaries were in neither git nor any backup. It is
    # ~70KB of scripts carrying a lot of hard-won tuning (the num_ctx ceiling, the
    # thinking-model fallbacks, the chunking fix). Keep this list short: a big
    # folder here would turn a per-commit backup into a long op.
    # AHK\ (2026-09-18) is gitignored and holds the live ALL NNN.ahk plus the
    # ytdl/OCR/transcribe/VECT/LLC helpers. It is ~945MB only because of test
    # downloads (YTD_claude\ 4K clips); $MediaExt below keeps those out, leaving
    # a few MB of scripts.
    [string[]]$Folders = @('ytsummaries', 'YTT', 'AHK'),
        ...
        'ok' {
            ...
            if ($code -lt 8) {
                ...
                foreach ($f in $Folders) {
                    ...
                    # /XD __pycache__: regenerated bytecode, and it would otherwise
                    # be most of YTT\'s file count for no recoverable value.
                    robocopy $src (Join-Path $t $f) /E /COPY:DAT /R:0 /W:0 /NP /NFL /NDL /NJH /NJS /XJ /XD '__pycache__' /XF @MediaExt | Out-Null
