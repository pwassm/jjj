    ...
    if (row.qfile) {
      ...
      if (ft.startsWith('[') || ft.startsWith('{')) {
        ...
        const _ftStyles =
          ...
          // (dev0951) White chip, fixed blue ink — see the long note on the
          // index.html copy. The slide-colour inherit rule went with it: on a
          // white ground it is what would paint a white-on-dark slide's icons
          // white on white.
...
function _vpTlFitWindow() {
  ...
  const ap = _vpState.aPoint, bp = _vpState.bPoint;
  if (ap == null || bp == null) return null;
  const a = Math.min(ap, bp), b = Math.max(ap, bp);
...
// Fraction across the bar (0–1) → time.
function _vpTlTimeAtFrac(f) {
  const w = _vpTlWindow();
